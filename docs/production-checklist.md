...
### H3. Per-coach rate override UI — `[x]`
...
- Est: 2 h.
- **Done:** Full pipeline. Schemas at [src/lib/schemas/rate-override.ts](../src/lib/schemas/rate-override.ts) (upsert + delete, single shape because the table's composite PK on `(coachId, resourceType)` makes upsert the natural API). Internal logic at [src/lib/server/rate-override-actions.ts](../src/lib/server/rate-override-actions.ts) uses Drizzle's `onConflictDoUpdate` for atomic upsert; audit logged with `entityType="rate_override"`, composite `entityId="${coachId}:${resourceType}"`, action="create"/"update"/"delete" inferred from pre-existing-row presence. Same safeLogAudit Sentry-capture pattern as session/block actions. Public wrappers at [src/app/admin/coaches/[id]/actions.ts](../src/app/admin/coaches/[id]/actions.ts) — both revalidate the coach detail page AND /admin/coaches (so the list-page "owed this month" recomputes). [form-actions.ts](../src/app/admin/coaches/[id]/form-actions.ts) handles FormData translation: dollars-string → cents-integer via a regex-validated parser (accepts "22", "22.50", optional `$` prefix; rejects negatives, letters, >2-decimal precision; multiplies before rounding to dodge float-drift at half-cent boundaries). UI: [_components/rate-overrides-card.tsx](../src/app/admin/coaches/[id]/_components/rate-overrides-card.tsx) is one row per resource type — each row has its own useActionState save form + a remove button (useTransition + confirm). Override rows show "Updated $DATE" via formatPfaDateMedium; default rows show "Using default" + the default rate as the input placeholder. Browser-verified all four flows end-to-end against a seeded coach: add new Bullpen override $25.50 → row remounts with Remove button visible; invalid "abc" input → validation error below input, value preserved; update existing Cage 18.00 → 20.00 → audit captured before/after diff; remove Cage → row reverts to "Using default" + Remove button gone. DB + audit log inspection confirmed all four ops landed correctly.

### H4. Admin audit log viewer — `[x]`
...
- Est: 2 h.
- **Done:** Server-rendered viewer at [/admin/audit](../src/app/admin/audit/page.tsx). URL-state filters (matches reports pattern) at [src/lib/audit/filters.ts](../src/lib/audit/filters.ts): date range (defaults to last 7 days — audit is a "what changed recently" surface, vs reports' month default), single actor dropdown (single not multi — drops a lot of complexity for tiny value), entity-type multi-checkbox (session / block / rate_override), action multi-checkbox (create / update / delete). Pagination via `?page=N` with 50/page; parallel queries for the page rows + count via [fetch.ts](../src/lib/audit/fetch.ts). Page sorted by ts desc. Table at [_components/audit-table.tsx](../src/app/admin/audit/_components/audit-table.tsx): When (PFA-zone date + time via formatPfaDate/Time), Actor (name + email fallback), Action (color-coded pill — success/gold/danger for create/update/delete), Entity (label + type-specific entityId rendering — rate_override splits the composite `${coachId}:${resourceType}` for human-readable display; session/block show truncated 8-char UUID prefix), Detail (native `<details>` with pretty-printed JSON diff — no JS needed). [Filter form](../src/app/admin/audit/_components/filters-form.tsx) uses GET-method native form (shareable URL, no client JS) with the "leave all unchecked = no filter" semantic + helper copy. Admin landing's "Block-off times" placeholder card replaced with a live Audit log link. Browser-verified with 5 seeded fixtures + the ~8 real audit rows from prior testing: viewer rendered 13 entries with all three entity types + all three actions visible; filtering to rate_override + delete narrowed to 3 entries correctly. **Closes Stage H.**
...
### J4b. Server-side timezone for date/time formatting — `[x]`
...
- **Done:** went with option (b) — code-level fix doesn't depend on Vercel env (no surprise if someone unsets it). New [src/lib/timezone.ts](../src/lib/timezone.ts) is the single source of truth: `PFA_TIMEZONE = "America/New_York"` constant + `formatPfaDate`/`formatPfaTime`/`formatPfaWeekday`/`formatPfaDateLong`/`formatPfaDateMedium`/`formatPfaMonthYear`/`pfaParts` helpers, all using `Intl.DateTimeFormat` with the explicit timeZone option. Refactored server-side surfaces: [aggregate.ts](../src/lib/reports/aggregate.ts) (date / dayOfWeek / startTime / endTime — affects Reports preview + Excel since Excel inherits via aggregate), [schedule/page.tsx](../src/app/admin/schedule/page.tsx) (dateLabel header), [coaches/page.tsx](../src/app/admin/coaches/page.tsx) (monthLabel), [coaches/[id]/page.tsx](../src/app/admin/coaches/[id]/page.tsx) (joined date). Aggregate test rewritten to use explicit `2026-05-01T13:00:00Z` UTC inputs (= 09:00 EDT) so display assertions are TZ-runtime-independent. **Proof:** ran `TZ=UTC npm test` (simulating Vercel runtime) → 37/37 pass with the same "09:00" / "10:00" assertions. The deeper TZ work (form input parsing + client-side display + date-bucketing math) tracked separately in J4f.

### J4f. End-to-end timezone rigor (form parsing + client display + bucketing) — `[x]`
- **Problem (deferred from J4b):** J4b fixed server-rendered display. The remaining surfaces still use runtime-local TZ:
  1. **Form input parsing.** `src/app/admin/sessions/form-actions.ts` (and the coach + block variants) build `new Date("${dateStr}T${timeStr}:00")` — parsed as runtime-local time. A coach in California entering "9 AM" stores a different UTC instant than a coach in NJ entering "9 AM". Same form input → different stored values.
  2. **Client-side display.** The schedule grid, sessions tables (admin + coach), edit dialog defaults — all use `getHours()` / `toLocaleDateString` without explicit TZ. Render in the viewer's browser TZ. A coach in California viewing the schedule sees PDT times for sessions entered in ET. Confusing.
  3. **Date-bucketing math.** `schedule/page.tsx` builds dayStart/dayEnd via `setHours(0,...)` (runtime-local). `coaches/page.tsx` builds month boundaries via `new Date(now.getFullYear(), now.getMonth(), 1)`. On Vercel UTC, a late-night ET session can land in the wrong day's / month's bucket.
- **Approach:** add `pfaMidnight(y, m, d)` and `parsePfaInput(dateStr, timeStr)` helpers to [src/lib/timezone.ts](../src/lib/timezone.ts) using the formatToParts-on-UTC-midnight pattern to compute the correct UTC offset for any given date (handles DST). Refactor form-actions to use parsePfaInput. Refactor client display helpers to use the formatPfa* utilities. Refactor server bucketing math to use pfaMidnight.
- **Acceptance:** (1) admin in NJ and admin on the road in California typing "9 AM 2026-05-24" produce the SAME stored UTC instant. (2) the same session row displays "9:00 AM" on both their screens. (3) monthly reports correctly include late-night ET sessions near month boundaries when viewed on Vercel UTC.
...
### J7. Privacy Policy + Terms of Service — `[x]`
