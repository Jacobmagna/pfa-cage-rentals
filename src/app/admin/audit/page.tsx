...
function PageLink({
  ...
  if (!href) {
    return (
      <span
        aria-disabled
        className="inline-flex items-center gap-1 rounded-md border border-line bg-surface px-3 h-9 text-fg-subtle opacity-40 cursor-not-allowed"
      >
  ...
  return (
    <Link
      href={href}
      className="inline-flex items-center gap-1 rounded-md border border-line bg-surface px-3 h-9 text-fg-muted hover:text-fg hover:border-line-strong transition-colors"
    >
      {dir === "prev" ? <Icon className="h-4 w-4" /> : null}
      {label}
      {dir === "next" ? <Icon className="h-4 w-4" /> : null}
