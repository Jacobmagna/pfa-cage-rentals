...
function FileInputRow({ disabled }: { disabled: boolean }) {
  return (
    <div className="flex flex-wrap items-end gap-3 rounded-xl border border-line bg-surface shadow-[var(--shadow-sm)] p-4">
      <div className="flex flex-col gap-1">
        <label htmlFor="xlsx" className="text-[11.5px] font-semibold uppercase tracking-[0.14em] text-fg-muted">
          Excel file
        </label>
        <input
          id="xlsx"
          ...
          accept=".xlsx"
          required
          className="text-sm text-fg file:mr-3 file:rounded-lg file:border file:border-line file:bg-surface-2 file:px-3 file:py-1.5 file:text-xs file:font-medium file:text-fg hover:file:border-line-strong"
        />
      </div>
      <button
        type="submit"
        ...
        value="preview"
        disabled={disabled}
        className="rounded-lg border border-line-strong bg-surface px-4 py-2 text-sm font-medium text-fg-muted hover:text-fg hover:-translate-y-px shadow-[var(--shadow-sm)] hover:shadow-[var(--shadow-md)] disabled:opacity-50 transition"
      >
...
function PreviewSection({
  ...
  return (
    <>
      <div className="rounded-xl border border-line bg-surface shadow-[var(--shadow-sm)] p-4">
        <p className="text-[11.5px] font-semibold uppercase tracking-[0.14em] text-fg-muted">Preview</p>
        <p className="mt-1 text-sm text-fg">
          <span className="font-medium text-gold-strong">{fileName}</span> — <span className="tnum tabular-nums">{totalParsed}</span> session
      ...
      <div className="overflow-x-auto rounded-xl border border-line bg-surface shadow-[var(--shadow-sm)]">
        <table className="w-full border-collapse text-sm">
          <thead className="bg-surface-2/50 border-b border-line text-left text-[11px] font-semibold uppercase tracking-wider text-fg-muted">
            <tr>
          ...
          <tbody>
            {groups.map((g) => {
              ...
              return (
                <tr key={g.rawName} className="border-t border-line hover:bg-surface-2 transition-colors">
                  <td className="px-3 py-2 font-mono text-xs text-fg">{g.rawName}</td>
                  <td className="px-3 py-2 text-fg">
                    {g.canonicalName || <span className="text-fg-subtle">—</span>}
                    {g.existingUserMatch && (
                      <span className="ml-1 text-[10px] font-medium uppercase tracking-wider text-gold-strong">
                        existing
                      </span>
                    )}
                  </td>
                  ...
                  <td className="px-3 py-2 text-right tnum tabular-nums text-fg-muted">{g.count}</td>
                  <td className="px-3 py-2">
                    <select
                      value={action}
                      onChange={(e) =>
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          value="commit"
          disabled={isPending}
          className="rounded-lg bg-gold px-4 py-2 text-sm font-medium text-gold-ink hover:bg-gold-hover shadow-[var(--shadow-sm)] disabled:opacity-50 transition-colors"
        >
...
function CommitSummary({ result }: { result: NonNullable<Extract<ImportFormState, { stage: "committed" }>["result"]> }) {
  return (
    <div className="space-y-4 rounded-xl border border-line bg-surface shadow-[var(--shadow-sm)] p-4">
      <div>
        <p className="text-[11.5px] font-semibold uppercase tracking-[0.14em] text-fg-muted">Import complete</p>
      ...
      {result.errored.length > 0 && (
        <details className="rounded-lg border border-danger/40 bg-danger/5 p-3 text-xs">
      ...
      <Link
        href="/admin/schedule"
        className="inline-block rounded-lg border border-line-strong bg-surface px-3 py-1.5 text-xs font-medium text-fg-muted hover:text-fg hover:-translate-y-px shadow-[var(--shadow-sm)] hover:shadow-[var(--shadow-md)] transition"
      >
        View schedule →
      </Link>
    </div>
...
function Stat({ label, value, accent }: { label: string; value: number; accent?: "gold" }) {
  return (
    <div>
      <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-fg-muted">{label}</p>
      <p className={`text-base font-semibold tnum tabular-nums ${accent === "gold" ? "text-gold-strong" : "text-fg"}`}>
        {value}
      </p>
    </div>
  );
...
function ConfidenceBadge({ confidence }: { confidence: GroupSummary["confidence"] }) {
  const map = {
    alias: { label: "alias", className: "ring-success/30 text-success bg-success/10" },
    fuzzy: { label: "fuzzy", className: "ring-warning/30 text-warning bg-warning/10" },
    cleaned: { label: "cleaned", className: "ring-gold/40 text-gold-strong bg-gold/10" },
    unmatched: { label: "unmatched", className: "ring-danger/30 text-danger bg-danger/10" },
  } as const;
  const m = map[confidence];
  return (
    <span
      className={`inline-flex items-center rounded-full ring-1 ring-inset px-2 py-0.5 text-[10px] font-medium uppercase tracking-wider ${m.className}`}
    >
      {m.label}
    </span>
  );
