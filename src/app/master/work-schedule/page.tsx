...
import { type ReconCoach } from "@/lib/server/reconciliation";
import {
  formatPfaDateLong,
  parsePfaInput,
  pfaDayEnd,
...
import { ProgramScheduleGrid } from "@/app/admin/hour-log/schedule/_components/program-schedule-grid";
import { MasterSubnav } from "../_components/master-subnav";

// Master work schedule (Add-On Part 1). Mirrors the admin programs schedule
// (/admin/hour-log/schedule) against the SAME live tables (two-way sync), but
// ships NO hour-log / accountability data: the reconciliation query + engine
// are stripped, so a schedule manager only sees + authors the admin-intended
// program blocks (program + scheduled coach + time + occupancy).
//
...
export default async function ProgramsSchedulePage({
  ...
  const [activePrograms, blockRows, coachRows, resourceRows] =
    await Promise.all([
    db
