...
import { pfaWeekRange, pfaMonthRange, formatPfaDate, PFA_TIMEZONE } from "./timezone";
...
describe("pfaWeekRange", () => {
  it("returns the Sunday→Sunday half-open range for a mid-week date", () => {
    // 2025-05-31 is a Saturday in PT.
  ...
  it("starts on the same Sunday when given a Sunday", () => {
    // 2025-05-25 is a Sunday in PT.
    const { startUtc } = pfaWeekRange("2025-05-25");
    expect(formatPfaDate(startUtc)).toBe("2025-05-25");
    expect(pfaWeekdayName(startUtc)).toBe("Sun");
  });

  it("week starts at PFA-local midnight (00:00 PT), not UTC midnight", () => {
    const { startUtc } = pfaWeekRange("2025-05-28");
    // Local midnight on 2025-05-25 (PDT, UTC-7) is 07:00 UTC.
    expect(startUtc.toISOString()).toBe("2025-05-25T07:00:00.000Z");
  });

  it("handles the spring DST boundary (week of 2025-03-09)", () => {
    // DST begins Sun 2025-03-09 in PT; that week is 1 hour short.
  ...
  it("handles the fall DST boundary (week of 2025-11-02)", () => {
    // DST ends Sun 2025-11-02 in PT; that week is 1 hour long.
  ...
  it("handles a week straddling the year boundary (Dec 2025 → Jan 2026)", () => {
    // 2025-12-31 is a Wednesday in PT; its week is Sun 12-28 → 2026-01-04.
...
describe("pfaMonthRange", () => {
  ...
  it("starts at PFA-local midnight (00:00 PT) on the 1st", () => {
    const { startUtc } = pfaMonthRange("2025-07-04");
    // Local midnight on 2025-07-01 (PDT, UTC-7) is 07:00 UTC.
    expect(startUtc.toISOString()).toBe("2025-07-01T07:00:00.000Z");
  });
});
